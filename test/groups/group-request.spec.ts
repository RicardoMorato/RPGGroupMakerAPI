...
test.group('Group Request', (group) => {
  ...
  test('It should return 404 when providing a nonexisting group for rejection', async (assert) => {
    const master = await UserFactory.create()
    const group = await GroupFactory.merge({ master: master.id }).create()
  ...
  test('It should return 404 when providing a nonexisting group request for rejection', async (assert) => {
    const master = await UserFactory.create()
    const group = await GroupFactory.merge({ master: master.id }).create()
